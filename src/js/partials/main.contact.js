(function() {
    ...
    app.controller('mainController', ['$scope', '$http', '$cacheFactory', '$httpParamSerializerJQLike', function($scope, $http, $httpParamSerializerJQLike, $cacheFactory) {
        ...
        if (!location[1]) location[1] = 'country';
        if (!location[1]) location[1] = 'commerc';
        ...
        function getJson() {
            if ($scope.currentTab == 2 && $scope.loadedTabs.indexOf(2) === -1) {
                $http.get('http://www.kre.ru/landing/sale/?suptype=city').success(function(data) {
                    $scope.loadedTabs.push(2);
                    data.sort(sort);
            ...
            } else if ($scope.currentTab == 3 && $scope.loadedTabs.indexOf(3) === -1) {
                $http.get('http://www.kre.ru/landing/sale/?suptype=country').success(function(data) {
                    $scope.loadedTabs.push(3);
                    data.sort(sort);
                    for(var i = 0; i < data.length; i += 1) {
                        var currentFlat = data[i];
            ...
            } else if ($scope.currentTab == 4 && $scope.loadedTabs.indexOf(4) === -1) {
                $http.get('http://www.kre.ru/landing/sale/?suptype=commercial').success(function(data) {
                    $scope.loadedTabs.push(4);
                    data.sort(sort);
                    for(var i = 0; i < data.length; i += 1) {
                        var currentFlat = data[i];
                        if (currentFlat.suptype == 'commerce') {
                            $scope.params.push({
                                id: currentFlat.id,
                                commerce: true,
